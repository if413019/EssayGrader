...
class Matrix {
  ...
  */
  ...
  // function __construct($params1, $params2, $params3) {
  //   //if( func_num_args() > 0 ) {
  //     $args = array();
  //     array_push($args, params1);
  //     array_push($args, params2);
  //     array_push($args, params3);
  //     $match = implode(",", array_map('gettype', $args));
  ...
  */
  ...
  function reducerow($rank) {
       $R = new Matrix($rank, $this->m); 
      for($i = 0; $i < $this->m; $i++)      
      for($j = 0; $j < $this->n; $j++)                  
      $R->set($j, $i, $this->A[$i][$j]);   
     return $R; 
  ...
  */ 
  function toHTML($width=2) { 
    ...
    for( $i = 0; $i < $this->m; $i++ ) {
      print( '<tr>' );
      for( $j = 0; $j < $this->n; $j++ )
        print( '<td style="background-color:#fff;border:1px solid red;padding:5px;text-align:center;vertical-align:middle;"><b>' . round($this->A[$i][$j],4) . '</b></td>' );
      print( '</tr>');
    }
    print( '</table>' );
  }
          ...
          function toHTMLA($rank) { 
    print( '<table style="background-color:#eee;">');
    for( $i = 0; $i < $rank; $i++ ) {
      print( '<tr>' );
      for( $j = 0; $j < $this->n; $j++ )
        print( '<td style="background-color:#fff;border:1px solid red;padding:5px;text-align:center;vertical-align:middle;"><b>' . round($this->A[$i][$j],5) . '</b></td>' );
      print( '</tr>');
    }
    print( '</table>' );
  }
